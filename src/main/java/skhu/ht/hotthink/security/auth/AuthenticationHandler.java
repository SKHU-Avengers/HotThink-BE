...
import skhu.ht.hotthink.security.model.error.TokenError;
...
public class AuthenticationHandler implements AuthenticationSuccessHandler, AuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException e) throws IOException, ServletException {
        ...
        if (e instanceof BadCredentialsException)
            mapper.writeValue(response.getWriter(), TokenError.of("비밀번호 틀림", TokenError.ErrorCode.AUTHENTICATION, HttpStatus.UNAUTHORIZED));
        else if (e instanceof JwtExpiredTokenException)
            mapper.writeValue(response.getWriter(), TokenError.of("Token has expired", TokenError.ErrorCode.JWT_TOKEN_EXPIRED, HttpStatus.UNAUTHORIZED));
...
//            mapper.writeValue(response.getWriter(), LoginError.of(e.getMessage(), LoginError.ErrorCode.AUTHENTICATION, HttpStatus.UNAUTHORIZED));
            ...
            mapper.writeValue(response.getWriter(), TokenError.of("유저를 찾을 수 없음", TokenError.ErrorCode.AUTHENTICATION, HttpStatus.UNAUTHORIZED));

        mapper.writeValue(response.getWriter(), TokenError.of("Authentication failed", TokenError.ErrorCode.AUTHENTICATION, HttpStatus.UNAUTHORIZED));
